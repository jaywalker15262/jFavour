package com.jay.favour.helpers
...
object ItemHelper {
    private val itemLogger = Logger.getLogger(this.javaClass.simpleName)
    ...
    fun <T> Item.useOnExtended(interactable: T, useMenu: Boolean? = null, useMenu2: Boolean? = null)
        ...
        if (!selectedItem.valid() || selectedItem.name() != name()) {
            itemLogger.info("JayLOGS: No selected item found or wrong selected item found.")
            if (selectedItem.valid()) {
                itemLogger.info("JayLOGS: Wrong selected item found, attempting to de-select it.")
